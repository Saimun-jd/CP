#include <bits/stdc++.h>
using namespace std;

#define fastio std::ios_base::sync_with_stdio(false); \
                 std::cin.tie(nullptr);

using vi = vector<int>;
using mii = map<int, int>;
using ll = long long;
using pii = std::pair<int, int>;
using vii = std::vector<pii>;
using WGraph = std::vector<std::vector<pii>>;
using Graph = std::vector<std::vector<int>>;
using MinHeap = std::priority_queue<pii, std::vector<pii>, std::greater<pii>>;
using ump = std::unordered_map<int, int>;

#define pb push_back
#define all(x) (x).begin(), (x).end()
#define sz(x) (x).size()
#define loop(i, n) for(int i=0; i<n; i++)
#define loop1(i, n) for(int i=0; i<=n; i++)
#define loop2(i, n) for(int i=1; i<=n; i++)
#define pn cout<< "NO" << endl;
#define py cout<< "YES" << endl;

const int MXN = 1e5 + 5, INF = 1e9 + 5;
const int N = 100;
const ll MOD = 1e9 + 7;

bool polarity(int x) {
    x = (x<0)? -x: x;
    return ((x)%2==1)? 1: 0;
}

void solve() {
	int tc;
	cin>> tc;
	while(tc--) {
		int n;
        cin>> n;
        vector<int> vec(n);
        int gloabl_max = INT_MIN; // maximum subarray sum
        int local_max = 0; // sum till i
        for(auto &x: vec) {
            cin>> x;
        }
        bool ok = 1;
        loop(i, n) {
            if(ok) {
                local_max = max(vec[i], vec[i]+local_max);
                gloabl_max = max(gloabl_max, local_max);
            } else {
                local_max = vec[i];
                gloabl_max = max(gloabl_max, local_max);
                ok = 1;
            }
            if(polarity(vec[i])^polarity(vec[i+1]) == 0) ok = 0;
        }
        
        cout<< gloabl_max << endl;
	}
}

signed main()
{
    fastio
    solve();
    
    return 0;
}
